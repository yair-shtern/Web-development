...
app.get("/random", (req, res) => {
  const randomIndx = Math.floor(Math.random() * jokes.length)
  res.json(jokes[randomIndx]);
});
...
app.get("/filter", (req, res) => {
  ...
  const jokesFilterd = jokes.filter((joke) => joke.jokeType === type)
  res.json(jokesFilterd);
});
